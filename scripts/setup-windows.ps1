...
<#
Install the VS 2022 17.4 toolset used with the pinned Google Clang 17 compiler.
...
if ($null -eq $toolset) {
    if ($VerifyOnly) { throw "MSVC $toolsetFamily is missing; run setup-windows.ps1 first." }
    ...
    Write-Host "Installing $component into $installation"
    ...
    $installerArguments = @('modify', '--installPath', "`"$installation`"", '--channelId', 'VisualStudio.17.Release', '--add', $component, '--quiet', '--norestart', '--nocache')
    ...
    $toolset = Find-PinnedToolset
    if ($null -eq $toolset) { throw "Installer did not provide the required $toolsetFamily toolset." }
}
...
int main() {
    ...
    return *count == 2 && path.filename() == "probe" ? 0 : 1;
}
